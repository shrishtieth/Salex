...
contract SaleXToken is ERC20, Ownable {
    ...
    uint256 public customBuyTax;
    uint256 public customSellTax;
    uint256 public AmountLiquidityFee;
    uint256 public AmountTokenRewardsFee;
    uint256 public AmountMarketingFee;
    uint256 public amountCustomFee;
    uint256 public swapEthThreshold = 10000000000000;
    uint256 public swapTokensThreshold = 100000000000;
    uint256 public buyBackAmount;
    uint256 public buyBackEthAmount;
    ...
    address public customFeeWallet;
    ...
    constructor(
        ...
        uint256[6] memory buyFeeSetting_, 
        uint256[6] memory sellFeeSetting_,
        ...
        rewardToken = addrs[0];
        minimumBalance = tokenBalanceForReward_;
        ...
        buyBuyBackFee = buyFeeSetting_[4];
        customBuyTax = buyFeeSetting_[5];
        ...
        sellBuyBackFee = sellFeeSetting_[4];
        customSellTax = sellFeeSetting_[5];

        require(buyTokenRewardsFee.add(buyLiquidityFee)
        .add(buyMarketingFee).add(buyDeadFee.add(customBuyTax)) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee)
        .add(sellMarketingFee).add(sellDeadFee.add(customSellTax)) <= 25, "Total sell fee is over 25%");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(AmountTokenRewardsFee > 0) {
               distributeReward(AmountTokenRewardsFee);
                }
            if(amountCustomFee >0){
                IERC20(address(this)).transfer(customFeeWallet, amountCustomFee);
            }
            swapping = false;
        }
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                BFee = amount.mul(buyBuyBackFee).div(100);
                buyBackAmount +=BFee;
                CFee = amount.mul(customBuyTax).div(100);
                amountCustomFee += CFee;
                fees = LFee.add(RFee).add(MFee).add(DFee).add(BFee).add(CFee);
            }
            if(automatedMarketMakerPairs[to]){
                ...
                BFee = amount.mul(sellBuyBackFee).div(100);
                buyBackAmount +=BFee;
                CFee = amount.mul(customSellTax).div(100);
                amountCustomFee += CFee;
                fees = LFee.add(RFee).add(MFee).add(DFee).add(BFee).add(CFee);
            
            }
            if(buyBackAmount >= swapEthThreshold){
                uint256 ethAmount = swapTokensForEth(buyBackAmount);
                 buyBackAmount = 0;
                 buyBackEthAmount += ethAmount;
            }
            if(buyBackEthAmount >= swapTokensThreshold){
                uint256 returnedAmount = swapEthForTokens(buyBackEthAmount);
                 _burn(address(this), returnedAmount);
                 buyBackEthAmount =0;
            }
    ...
    function distributeReward(uint256 amount) private rebasing{

        uint256 total = allHolders.length;
        uint256 supply = totalSupply();
            for(uint256 i=0; i< total; i++){
                if(balanceOf(allHolders[i])>0)
            IERC20(address(this)).transfer(allHolders[i],(balanceOf(allHolders[i])*amount)/supply);
        }            
